...
function Loader(divid, options)
{
	...
	this.Start = function()
	{
		...
		for (var i = 0; i < this.images.length; i++) {
			var imgLoad = $("<img></img>");
			$(imgLoad).prop("src", this.images[i]);
			$(imgLoad).unbind("load");
	...
	this.getImages = function()
	{
		...
		else if (typeof($(this.div_id).prop("src")) != "undefined" && $(this.div_id).prop("tagName") == "img") {
				url = $(this.div_id).prop("src");
		}
		...
		var everything = $(this.div_id).find("*:not(script)").each(function() {
			...
			} else if (typeof($(this).prop("src")) != "undefined" && $(this).prop("tagName") == "img") {
				url = $(this).prop("src");
			}
